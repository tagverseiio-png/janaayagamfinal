...
export default function PortalLayout({ children, sidebarLinks, roleLabel }) {
 ...
 return (
 <div className="min-h-screen bg-[#F0EBE3] text-slate-800 flex flex-col font-sans transition-colors duration-300">
 
 {/* ══ MOBILE VIEW HEADER (fixed, height 56px) ══ */}
 <header className="md:hidden fixed top-0 left-0 right-0 h-14 bg-white border-b border-slate-200 px-4 flex justify-between items-center z-50">
 <button
 onClick={() => setMobileMenuOpen(true)}
 className="w-11 h-11 flex items-center justify-center rounded-xl hover:bg-slate-50 text-slate-700 "
 >
 ...
 <button className="w-11 h-11 flex items-center justify-center rounded-full hover:bg-slate-50 text-slate-500 relative">
 ...
 {/* Drawer sheet */}
 <div className="relative w-[280px] max-w-[280px] bg-white h-full flex flex-col shadow-2xl z-10 transition-transform duration-250 ease-out translate-x-0">
 {/* Drawer top close */}
 <div className="p-4 flex justify-end">
 <button 
 onClick={() => setMobileMenuOpen(false)}
 className="w-11 h-11 flex items-center justify-center text-slate-400 hover:text-slate-600 "
 >
 <X className="w-6 h-6" />
 </button>
 </div>
 ...
 <h2 className="text-sm font-extrabold text-slate-800 uppercase tracking-wide truncate max-w-full">
 {userName}
 </h2>
 <span className="px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase bg-red-50 text-[#8B1A1A] border border-[#8B1A1A]/10">
 {role.replace('_', ' ')}
 </span>
 ...
 isActive
 ? 'bg-red-50 text-[#8B1A1A] border-l-4 border-[#8B1A1A]'
 ...
 {/* Left Sidebar */}
 <aside className="w-[240px] bg-white border-r border-slate-200 flex flex-col justify-between select-none shrink-0 z-30">
 ...
 <button
 onClick={handleLogout}
 className="w-full flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-rose-50 hover:bg-rose-100 text-[10px] font-black text-rose-600 uppercase border border-rose-100 "
 >
 ...
 <header className="h-14 bg-white border-b border-slate-200 px-6 flex justify-between items-center select-none shrink-0 z-20">
 ...
 <nav className="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-white border-t border-slate-200 flex justify-around items-center px-1 z-40 pb-[env(safe-area-inset-bottom)] fixed-bottom-nav select-none shadow-[0_-4px_16px_rgba(0,0,0,0.03)]">
 {activeTabs.map((tab) => {
