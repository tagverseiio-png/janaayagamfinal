...
export default function MlaDashboard({ overviewMode = false }) {
  const { t, lang, toggleLang } = useLanguage();
  const navigate = useNavigate();
  
  const userName = localStorage.getItem('jn_name') || 'MLA Name';
  const district = localStorage.getItem('jn_emp_district') || 'Chennai';
  const constituency = localStorage.getItem('jn_emp_constituency') || 'Velachery';
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error('Error fetching MLA dashboard data:', err);
      }
    };
    fetchData();
  ...
  const generateReport = () => {
    const reportContent = `JANANAYAGAM â€” CABINET REPORT
        ...
        <div>
          ...
          <div className="p-5 border-b border-slate-200 flex items-center gap-2">
            ...
            <div>
              ...
              <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1 block">MLA COMMAND CENTER</span>
            </div>
          </div>
          {/* Menu Items */}
        ...
        <div className="p-4 border-t border-slate-200 space-y-3">
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[10px] text-slate-400 font-bold uppercase">MLA</span>
            </div>
          </div>
          <div className="flex gap-2">
             ...
             <button onClick={() => toggleLang('ta')} className={`flex-1 py-1 text-[10px] font-black rounded ${lang === 'ta' ? 'bg-[#8B1A1A] text-white' : 'bg-slate-100 text-slate-500'}`}>à®¤à®®à®¿à®´à¯</button>
          </div>
      ...
      <div className="bg-white border-l-8 border-l-[#FF6600] border-t-4 border-t-[#138808] rounded-3xl p-6 shadow-sm flex justify-between items-center">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <h1 className="text-3xl font-black text-[#8B1A1A] mt-1 leading-none drop-shadow-sm">MLA Command Center</h1>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-3xl p-6 shadow-sm border border-slate-200">
          <div className="flex justify-between items-center mb-6">
            <h3 className="font-black text-lg text-[#8B1A1A] uppercase tracking-wide flex items-center gap-2">
              <MapPin className="w-5 h-5" /> Jurisdiction Command View
            </h3>
    ...
    <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-200 h-full flex flex-col">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-black text-[#8B1A1A] uppercase tracking-wide flex items-center gap-2">
          <FileText className="w-6 h-6" /> Constituency Tickets Registry
        </h2>
  ...
  const renderAnalytics = () => (
    <div className="space-y-6">
      <h2 className="text-xl font-black text-[#8B1A1A] uppercase tracking-wide flex items-center gap-2 mb-4">
        <TrendingUp className="w-6 h-6" /> Jurisdiction Analytics & Performance
      </h2>
        ...
        {reportGenerated && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-8 text-left">
            <div className="bg-emerald-50 text-emerald-700 px-4 py-3 rounded-lg font-bold text-sm mb-4 flex items-center gap-2 justify-center">
              âœ… Cabinet Report generated and downloaded successfully
            </div>
