...
export default function MlaAnalytics() {
 ...
 { name: t('categories.water'), count: waterCount, color: 'bg-[#8B1A1A]' },
 ...
 { name: t('categories.welfare'), count: welfareCount, color: 'bg-[#8B1A1A]' },
 ...
 row.isWorst 
 ? 'bg-rose-50/70 text-rose-700 font-extrabold border-l-4 border-l-rose-500' 
 ...
 {row.officer}
 {row.isWorst && (
 <span className="ml-2 bg-rose-200 text-rose-700 text-[9px] font-black uppercase px-2 py-0.5 rounded-full">
 Worst Rating
 </span>
 )}
 </td>
 ...
 <div 
 key={item.rank}
 className="flex items-start gap-4 p-4 bg-slate-50 border rounded-2xl shadow-sm hover:border-[#8B1A1A]/45 transition-colors"
 >
 <div className="w-10 h-10 bg-slate-100 border rounded-full flex items-center justify-center shrink-0 text-base font-black text-slate-700 ">
 #{item.rank}
 </div>
 ...
 <span className="text-xs text-[#8B1A1A] font-bold shrink-0">{item.count} grievances</span>
 </div>
