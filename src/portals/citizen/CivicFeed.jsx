...
export default function CivicFeed() {
  ...
  const storiesList = [
    ...
    ...departments.slice(0, 5).map(d => ({
      id: d.slug || d.name.toLowerCase().split(' ')[0],
  ...
  const handleStoryTap = (storyId) => {
    ...
    if (activeStory === storyId) {
      ...
      if (storyId === 'critical') {
        ...
        const dept = departments.find(d => d.slug === storyId || d.name.toLowerCase().startsWith(storyId));
          ...
          return (
            ...
            >
              ...
              <div className="relative w-full aspect-video md:aspect-square bg-slate-100 group">
                {item.photo ? (
                  <img src={getMediaUrl(item.photo)} alt="Issue proof" className="w-full h-full object-cover" />
                ) : (
