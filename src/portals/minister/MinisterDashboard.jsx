...
import { 
  ministerKpis, hierarchyStats, ministerOfficers, escalations,
  districts, aiInsights, infrastructure, trends, mapMarkers 
} from '../../mock';
...
export default function MinisterDashboard() {
  ...
  const criticalCount = escalations.filter(e => e.status === 'critical').length;
  ...
  const getStatusBadge = (status) => {
    if (status === 'critical') return <span className="bg-rose-100 text-rose-700 border border-rose-200 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-widest animate-pulse">Critical</span>;
    if (status === 'active') return <span className="bg-amber-100 text-amber-700 border border-amber-200 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-widest">Active</span>;
  ...
  return (
    <div className="min-h-screen bg-[#F0EBE3] text-slate-800 font-sans overflow-x-hidden relative pb-20">
      ...
      <main className="max-w-[1600px] mx-auto px-6 py-8 relative z-10 space-y-10">
        ...
        <section>
          <p className="text-[10px] font-black tracking-widest uppercase text-slate-500 mb-1">State Electricity Command</p>
          <h1 className="text-3xl font-black text-slate-800 tracking-tight flex items-center gap-2 mb-6">
            ...
            Electricity Minister Dashboard
          </h1>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            ...
            <div className="bg-white border border-slate-200 shadow-sm rounded-2xl p-5 flex flex-col justify-between hover:shadow-md transition-shadow relative overflow-hidden">
              ...
              <div>
                <div className="text-3xl font-black text-slate-800 mb-1">{ministerKpis.totalGrievances.toLocaleString()}</div>
                <div className="flex items-center gap-1 text-[10px] font-bold text-emerald-600">
                  <TrendingDown className="w-3 h-3" />
                  <span>{ministerKpis.prevPct}% vs last month</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white border border-slate-200 shadow-sm rounded-2xl p-5 flex flex-col justify-between hover:shadow-md transition-shadow">
              ...
              <div>
                <div className="text-3xl font-black text-slate-800 mb-1">{ministerKpis.resolutionRate}%</div>
                <div className="text-[10px] font-medium text-slate-500 uppercase tracking-widest">
                  {ministerKpis.resolved.toLocaleString()} resolved
                </div>
              </div>
            </div>
            ...
            <div className="bg-white border border-rose-200 shadow-sm rounded-2xl p-5 flex flex-col justify-between hover:shadow-md transition-shadow">
              <div className="flex justify-between items-start mb-4">
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Active Outages</span>
                <AlertTriangle className="w-4 h-4 text-rose-500" />
              </div>
              <div>
                <div className="text-3xl font-black text-rose-600 mb-1">{ministerKpis.activeOutages}</div>
                <div className="text-[10px] font-bold text-rose-500 uppercase tracking-widest">
                  {ministerKpis.householdsAffected.toLocaleString()} HH Affected
                </div>
              </div>
            </div>
            ...
            <div className="bg-white border border-slate-200 shadow-sm rounded-2xl p-5 flex flex-col justify-between hover:shadow-md transition-shadow">
              ...
              <div>
                <div className="text-3xl font-black text-amber-600 mb-1">{ministerKpis.criticalEscalations}</div>
                <div className="text-[10px] font-medium text-slate-500 uppercase tracking-widest">
                  Dir & Min Level
                </div>
              </div>
            ...
            <div className="bg-white border border-slate-200 shadow-sm rounded-2xl p-5 flex flex-col justify-between hover:shadow-md transition-shadow">
              <div className="flex justify-between items-start mb-4">
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Restoration</span>
                <ClockIcon className="w-4 h-4 text-slate-400" />
              </div>
              <div>
                <div className="text-3xl font-black text-slate-800 mb-1">{ministerKpis.avgRestorationHrs}<span className="text-lg text-slate-400 ml-1">hrs</span></div>
                <div className="flex items-center gap-1 text-[10px] font-bold text-emerald-600">
                  <TrendingDown className="w-3 h-3" />
                  <span>{ministerKpis.restorationTrend} trend</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white border border-slate-200 shadow-sm rounded-2xl p-5 flex flex-col justify-between hover:shadow-md transition-shadow">
              <div className="flex justify-between items-start mb-4">
                ...
                {renderScoreDot(ministerKpis.scoreStatus)}
              </div>
              <div>
                <div className="text-3xl font-black text-slate-800 mb-1">{ministerKpis.deptScore}<span className="text-lg text-slate-400 ml-1">/100</span></div>
                <div className="text-[10px] font-medium text-slate-500 uppercase tracking-widest">
                  Department wide
                </div>
              </div>
                ...
                <React.Fragment key={level.id}>
                  ...
                  >
                    <div className="flex justify-between items-center mb-4">
                      <div className="text-xs font-black text-slate-800 uppercase tracking-wider">{level.levelName}</div>
                    ...
                    <div className="grid grid-cols-2 gap-y-3 gap-x-2">
                      ...
                      <div>
                        <div className="text-[9px] text-slate-400 uppercase tracking-widest mb-0.5">Resp. Time</div>
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="text-sm text-slate-700">
                          {ministerOfficers.filter(o => o.parentOfficerId === selectedLevel.id).map(officer => (
                          ...
                          {ministerOfficers.filter(o => o.parentOfficerId === selectedLevel.id).length === 0 && (
                            <tr>
                              <td colSpan="5" className="py-8 text-center text-xs text-slate-400 uppercase tracking-widest">
                                No officers found for this level in mock data
                              </td>
                            </tr>
                          )}
                        </tbody>
        ...
        <div className="grid grid-cols-3 gap-6">
          ...
          <section className="col-span-2 space-y-4">
            ...
            <div className="space-y-4">
              {escalations.map((esc) => {
                const officer = ministerOfficers.find(o => o.id === esc.officerId) || {};
                return (
                  <div key={esc.id} className="bg-white border border-slate-200 shadow-sm rounded-2xl p-5 flex gap-5 hover:shadow-md transition-shadow border-l-4 border-l-rose-400">
                    ...
                    <div className="flex-1 space-y-3">
                      ...
                      <div className="flex items-center justify-between pt-2 border-t border-slate-100">
                        <div className="flex items-center gap-2 text-xs">
                          ...
                          <span className="font-black text-[#8B1A1A]">{officer.name || esc.officerId}</span>
                          <span className="text-[10px] bg-slate-100 px-1.5 py-0.5 rounded text-slate-500">{officer.role}</span>
                        </div>

                        <div className="flex items-center gap-2">
                          <button className="text-[10px] font-black uppercase tracking-widest bg-[#8B1A1A]/10 text-[#8B1A1A] hover:bg-[#8B1A1A]/20 px-3 py-1.5 rounded transition-colors flex items-center gap-1 border border-[#8B1A1A]/20">
                            <CornerUpRight className="w-3 h-3" /> Direction
                          </button>
                          <button className="text-[10px] font-black uppercase tracking-widest bg-rose-50 text-rose-600 hover:bg-rose-100 px-3 py-1.5 rounded transition-colors flex items-center gap-1 border border-rose-200">
                            <Activity className="w-3 h-3" /> Mark Urgent
                          </button>
                          <button className="text-[10px] font-black uppercase tracking-widest bg-slate-50 text-slate-600 hover:bg-slate-100 px-3 py-1.5 rounded transition-colors border border-slate-200">
                            Track ATR
                          </button>
                        </div>
                      </div>
          ...
          <section className="space-y-4">
            ...
            <div className="bg-white border border-slate-200 shadow-sm rounded-2xl overflow-hidden">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="text-sm text-slate-700">
                  {districts.sort((a,b) => b.resolutionPct - a.resolutionPct).map(dist => (
                    <tr key={dist.name} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                      ...
                      <td className="py-3 px-4 text-right">
                        <span className={`font-mono font-bold ${dist.resolutionPct > 80 ? 'text-emerald-600' : dist.resolutionPct > 70 ? 'text-amber-600' : 'text-rose-600'}`}>
                          {dist.resolutionPct}%
                        </span>
                      </td>
                      <td className="py-3 px-4 text-right font-mono">{dist.avgClosureHrs}h</td>
                      <td className="py-3 px-4 text-center flex justify-center mt-1">
                        {renderScoreDot(dist.status)}
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <section className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-white border border-slate-200 shadow-sm rounded-2xl p-5 flex flex-col gap-4">
                ...
                <div>
                  <div className="text-3xl font-black text-slate-800">{infrastructure.transformers.total.toLocaleString()}</div>
                ...
                <div className="space-y-2 mt-2">
                  <div className="flex justify-between text-xs border-b border-slate-100 pb-1">
                    ...
                    <span className="font-mono text-rose-600 font-black">{infrastructure.transformers.faulty}</span>
                  </div>
                  <div className="flex justify-between text-xs border-b border-slate-100 pb-1">
                    <span className="text-slate-500">Maintenance Due</span>
                    <span className="font-mono text-amber-600 font-black">{infrastructure.transformers.maintenanceDue}</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-slate-500">High Risk</span>
                    <span className="font-mono text-rose-700 font-black">{infrastructure.transformers.highRisk}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="bg-white border border-slate-200 shadow-sm rounded-2xl p-4 flex justify-between items-center">
                  <div>
                    ...
                    <div className="text-2xl font-black text-slate-800">{infrastructure.substations.active} <span className="text-[10px] text-emerald-600 uppercase tracking-widest font-black">Active</span></div>
                  </div>
                  <div className="text-right">
                    <div className="text-[10px] uppercase tracking-widest text-slate-400 mb-1">Avg Score</div>
                    <div className="text-xl font-mono text-[#8B1A1A] font-black">{infrastructure.substations.avgScore}/100</div>
                  </div>
                </div>
                ...
                <div className="bg-white border border-slate-200 shadow-sm rounded-2xl p-4 flex justify-between items-center">
                  <div>
                    ...
                    <div className="text-xl font-black text-slate-800">{infrastructure.electricLines.damagedReports} <span className="text-[10px] text-rose-600 uppercase tracking-widest font-black">Damaged</span></div>
                  </div>
                  <div className="text-right">
                    <div className="text-[10px] uppercase tracking-widest text-slate-400 mb-1">Repair Ongoing</div>
                    <div className="text-xl font-mono text-amber-600 font-black">{infrastructure.electricLines.repairOngoing}</div>
                  </div>
                </div>
              </div>
            </div>
